...
class TrainPipeline():
    def __init__(self, init_model=None,transfer_model=None):
        ...
        if (init_model is not None) and os.path.exists(init_model+'.index'):
            ...
            self.policy_value_net = PolicyValueNet(self.board_width,self.board_height,block=self.resnet_block,init_model=init_model,cuda=False)
            ...
            self.policy_value_net = PolicyValueNet(self.board_width,self.board_height,block=self.resnet_block,transfer_model=transfer_model,cuda=False)
        else:
            # start training from a new policy-value net
            self.policy_value_net = PolicyValueNet(self.board_width,self.board_height,block=self.resnet_block,cuda=False)
    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if (i+1) % 1000 == 0:
                    self.policy_value_net.save_model('tmp/current_policy' + str(i) + '.model')
                    print("Total sequence for losses at" + str(i) + "th game is")
                    print(losses)
                    print()
                    print("Total sequence for entropies at" + str(i) + "th game is")
                    print(entropies)
                    print()
                ...
                if (i+1) % self.check_freq == 0 :
                    ...
                    if (i+1) % (self.check_freq*2) == 0:
                        ...
                        logging.info("Winning ratio is {}".format(win_ratio))
...
if __name__ == '__main__':
    now = time.time()//1000
    logging.basicConfig(level=logging.DEBUG, filename="logfile"+str(now), filemode="a+",
                        format="%(asctime)-15s %(levelname)-8s %(message)s")
    training_pipeline = TrainPipeline(init_model='current_policy.model',transfer_model=None)
